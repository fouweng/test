...
#define MSG_MASTER_FILE	"octopus.msg"
...
#define dump_cmd_msg(cmd)	\
	do {\
		opt_log(OPT_LOG_FINE, "msg type:%d\n", (int) cmd->mtype); \
		if (cmd->domain == CMD_DOMAIN_CTL) { \
			struct ctl_msg_p *ctl = \
				(struct ctl_msg_p *) cmd->priv; \
			opt_log(OPT_LOG_FINE, "CTL cmd:%d pid:%d\n", \
				ctl->cmd, cmd->pid); \
			opt_log(OPT_LOG_FINE, "CTL msg_file:%s dev_name:%s\n", \
				ctl->msg_file, ctl->dev_name); \
		} else if (cmd->domain == CMD_DOMAIN_V4L2) { \
			uint32_t request = *((uint32_t *) (cmd->priv)); \
			...
			uint8_t *request = ((uint8_t*) (cmd->priv)); \
